...
public class DefaultSVGWriter implements SVGWriter {
    ...
    protected void drawGrid(VoltageLevelGraph graph, Document document, GraphMetadata metadata, Element root) {
        ...
        String gridId = idProvider.createSvgId(graph.getVoltageLevelInfos().id(), "GRID");
    ...
    protected void drawBuses(Element root,
        ...
        for (BusNode busNode : graph.getNodeBuses()) {
            ...
            String nodeId = idProvider.createSvgId(busNode.getId());
    ...
    protected void drawNodes(Element root,
        ...
        for (Node node : nodes) {
            String nodeSvgId = idProvider.createSvgId(node.getId());
            Element g = root.getOwnerDocument().createElement(GROUP);
            ...
            setMetadata(metadata, node, nodeSvgId, graph, direction, nodeLabels, nodeLabelsMetadata);
        }
    }

    protected void setMetadata(GraphMetadata metadata, Node node, String nodeEscapedId, BaseGraph graph, Direction direction, List<LabelProvider.NodeLabel> nodeLabels, List<GraphMetadata.NodeLabelMetadata> nodeLabelsMetadata) {
        String nextVId = null;
    ...
    protected void drawNodeLabel(Element g, Node node, List<LabelProvider.NodeLabel> nodeLabels, List<GraphMetadata.NodeLabelMetadata> nodeLabelsMetadata) {
        for (LabelProvider.NodeLabel nodeLabel : nodeLabels) {
            ...
            String svgId = idProvider.createSvgId(node.getId(), labelPosition.positionName());
    ...
    protected void insertComponentSVGIntoDocumentSVG(String componentType, Element g, Graph graph, Node node,
        ...
        BiConsumer<Element, String> elementAttributesSetter
                = (elt, subComponent) -> setComponentAttributes(g, graph, node, styleProvider, elt, componentType, subComponent);
    ...
    protected void insertDecoratorSVGIntoDocumentSVG(LabelProvider.NodeDecorator nodeDecorator,
        ...
        BiConsumer<Element, String> elementAttributesSetter
                = (elt, subComponent) -> setDecoratorAttributes(g, graph, node, nodeDecorator, styleProvider, elt, subComponent);
    ...
    private void setComponentAttributes(Element g, Graph graph, Node node, StyleProvider styleProvider,
    ...
    private void setDecoratorAttributes(Element g, Graph graph, Node node, LabelProvider.NodeDecorator nodeDecorator,
    ...
    private void drawFeederInfo(FeederNode feederNode, List<Point> points, Element root,
        ...
        String svgId = idProvider.createSvgId(feederNode.getId(), feederInfo.getComponentType());
    ...
    private void drawBusInfo(BusNode busNode, Element root, BusInfo busInfo,
        ...
        // Identity
        String svgId = idProvider.createSvgId(busNode.getId(), busInfo.getComponentType());
        g.setAttribute("id", svgId);
    ...
    protected void drawEdges(Element root, VoltageLevelGraph graph, GraphMetadata metadata,
        ...
        for (Edge edge : edges) {
            String wireId = idProvider.createSvgId(voltageLevelId, edge.getNode1().getId(), edge.getNode2().getId());
    ...
    private void drawSnakeLines(Graph graph, BranchEdge edge, Element root, GraphMetadata metadata, StyleProvider styleProvider) {
        Element g = root.getOwnerDocument().createElement(GROUP);
        String snakeLineId = idProvider.createSvgId(edge.getId());
